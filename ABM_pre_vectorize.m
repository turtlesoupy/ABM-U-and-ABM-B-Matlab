function [reflectance, transmittance, absorptance] = ABM_pre_vectorize(azimuthalI, polarI, interfaceArray, nSamples)
    stepFunction  = @step;    
    startState    = 1;
    ...
    parfor i = 1:nSamples
        ...
        [state, direction] = stepFunction(state, direction);
    ...
    function [outState, outVector] = step(state, vector)
        ...
        if interface.thickness > 0 && ...
                freePathLength(vector, normal, normalAngle, interface.absorptionCoefficient) < interface.thickness
