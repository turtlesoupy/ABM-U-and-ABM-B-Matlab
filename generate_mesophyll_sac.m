...
sample.proteinFraction = 0.2963;
sample.ligninFraction = 0.2655;
...
sample.airVolumeFraction = 0.127;
sample.spongyCellCapsAspectRatio = obs;
nSamples = 100000;
wavelengths = 750e-9:5e-9:2500e-9;
...
mesophyllSAC = zeros(1, nWavelengths);

for n = 1:length(wavelengths)
    tic
    fprintf('Wavelength %d\n', wavelengths(n))
    
    interfaces = build_abm_infrared_interfaces(sample, wavelengths(n), 0);
    mesophyllSAC(n) = interfaces(2).absorptionBelow;
end
toc
